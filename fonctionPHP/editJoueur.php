<?php
	...
	$req = $linkpdo->prepare('UPDATE joueur set numero_licence = :numero_licence, nom = :nom, prenom = :prenom, photo = :photo, date_naissance = :date_naissance, taille = :taille , poids = :poids , poste_prefere = :poste_prefere, note_perso = :note_perso , statut = :statut WHERE numero_licence = :numero_licence');
	$recupimg = $linkpdo->prepare('SELECT photo FROM joueur WHERE numero_licence = :numero_licence');
    $recupimg->execute(array('numero_licence' => $numero_licence));
    $img = $recupimg->fetch();
